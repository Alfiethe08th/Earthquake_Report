    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for(int i = 0; i<jsonArray.length(); i++)
            {
                ...
                int positionofOf = location.indexOf("of ");
                int locSize = location.length();
                long time = properties.getLong("time");

                if(location.contains("of"))
                {
                    locPrime = location.substring(0, (positionofOf+2));
                    locOffSet = location.substring((positionofOf+3), locSize);
                    Earthquake earthquake = new Earthquake(magnitude, locPrime, locOffSet, time);
                    earthquakes.add(earthquake);
                }
                else
                    {
                        locPrimeSub = "Near the";
                        locOffSet = properties.getString("place");
                        Earthquake earthquake = new Earthquake(magnitude, locPrimeSub, locOffSet, time);
                        earthquakes.add(earthquake);
                    }
